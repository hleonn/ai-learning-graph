...
export default function CurriculumGenerator() {
    ...
    const saveCourseWithProgress = async (existingCourseId?: string): Promise<string> => {
        ...
        const totalOperations = totalSubtopics * 2
        ...
        // Crear nodos (actualizar progreso)
        for (const phase of roadmap.phases) {
            for (const topic of phase.topics) {
                for (const subtopic of topic.subtopics) {
        ...
        // Crear edges (actualizar progreso)
        for (const phase of roadmap.phases) {
            for (const topic of phase.topics) {
                for (const subtopic of topic.subtopics) {
    ...
    const createClassroomMaterials = async (classroomCourseId: string) => {
        ...
        // Calcular total de materiales a crear
...
`
                    ...
                    // Actualizar progreso después de cada material
                    ...
                    <div style={s.resultPanel}>
                        ...
                        {publishProgress && (
                            <div style={s.progressBarContainer}>
                                ...
                                <span style={s.progressText}>
                                    📚 Publicando en Classroom: {Math.round((publishProgress.current / publishProgress.total) * 100)}%
                                    ({publishProgress.current}/{publishProgress.total} materiales)
                                </span>
                            </div>
                        )}
...
const s: Record<string, React.CSSProperties> = {
    ...
    body: { display: 'flex', gap: 24, padding: 24, alignItems: 'flex-start', flexWrap: 'wrap', flex: 1, overflow: 'hidden', minHeight: 0, },
    ...
    resultPanel: { flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 20, height: '100%', overflow: 'hidden', minHeight: 0, },
