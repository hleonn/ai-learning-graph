...
class EdgeCreate(BaseModel):
    source_id: str  # Cambiado: ahora es el ID del nodo origen
    target_id: str  # Cambiado: ahora es el ID del nodo destino
    prerequisite_strength: float = 0.8
    edge_type: str = "prerequisite"
...
def create_node(course_id: str, node: NodeCreate):
    ...
    # Verificar que no exista un nodo con el mismo label en el mismo curso
    existing = supabase.table("concept_nodes").select("id").eq("course_id", course_id).eq("label", node.label).execute()
    if existing.data:
        logger.info(f"Nodo ya existe con label '{node.label}', usando el existente")
...
def create_edge(course_id: str, edge: EdgeCreate):
    """Crear una nueva arista (prerrequisito) en el curso"""
    logger.info(f"Creando arista en curso {course_id}: {edge.source_id} -> {edge.target_id}")
    ...
    # Verificar que los nodos existen por ID
    source_res = supabase.table("concept_nodes").select("id").eq("course_id", course_id).eq("id", edge.source_id).execute()
    target_res = supabase.table("concept_nodes").select("id").eq("course_id", course_id).eq("id", edge.target_id).execute()

    if not source_res.data:
        raise HTTPException(status_code=404, detail=f"Nodo origen con ID '{edge.source_id}' no encontrado")
    if not target_res.data:
        raise HTTPException(status_code=404, detail=f"Nodo destino con ID '{edge.target_id}' no encontrado")

    # Verificar que no exista ya un edge igual para evitar duplicados
    existing = supabase.table("concept_edges").select("id").eq("course_id", course_id).eq("source_id", edge.source_id).eq("target_id", edge.target_id).execute()
    if existing.data:
        logger.info(f"Edge ya existe, omitiendo: {edge.source_id} -> {edge.target_id}")
        return existing.data[0]
...
def update_node_position(course_id: str, node_id: str, position: dict = Body(...)):
    ...
    node_res = supabase.table("concept_nodes").select("id").eq("id", node_id).eq("course_id", course_id).execute()
